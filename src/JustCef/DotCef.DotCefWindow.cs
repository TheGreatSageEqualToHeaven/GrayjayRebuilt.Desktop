...
{
    private readonly TaskCompletionSource _closeCompletionSource = new();
    ...
    public DotCefWindow(DotCefProcess process, int identifier,
        Func<DotCefWindow, IPCRequest, IPCRequest?>? requestModifier,
        Func<DotCefWindow, IPCRequest, Task<IPCResponse?>>? requestProxy)
    {
        _process = process;
    ...
    public async Task MaximizeAsync(CancellationToken cancellationToken = default)
    {
        await _process.WindowMaximizeAsync(Identifier, cancellationToken);
    }

    public async Task MinimizeAsync(CancellationToken cancellationToken = default)
    {
        await _process.WindowMinimizeAsync(Identifier, cancellationToken);
    }

    public async Task RestoreAsync(CancellationToken cancellationToken = default)
    {
        await _process.WindowRestoreAsync(Identifier, cancellationToken);
    }

    public async Task ShowAsync(CancellationToken cancellationToken = default)
    {
        await _process.WindowShowAsync(Identifier, cancellationToken);
    }

    public async Task HideAsync(CancellationToken cancellationToken = default)
    {
        await _process.WindowHideAsync(Identifier, cancellationToken);
    }

    public async Task ActivateAsync(CancellationToken cancellationToken = default)
    {
        await _process.WindowActivateAsync(Identifier, cancellationToken);
    }

    public async Task BringToTopAsync(CancellationToken cancellationToken = default)
    {
        await _process.WindowBringToTopAsync(Identifier, cancellationToken);
    }

    public async Task SetAlwaysOnTopAsync(bool alwaysOnTop, CancellationToken cancellationToken = default)
    {
        await _process.WindowSetAlwaysOnTopAsync(Identifier, alwaysOnTop, cancellationToken);
    }
    ...
    public async Task SetPositionAsync(int x, int y, CancellationToken cancellationToken = default)
    {
        await _process.WindowSetPositionAsync(Identifier, x, y, cancellationToken);
    }

    public async Task<(int X, int Y)> GetPositionAsync(CancellationToken cancellationToken = default)
    {
        return await _process.WindowGetPositionAsync(Identifier, cancellationToken);
    }

    public async Task SetSizeAsync(int width, int height, CancellationToken cancellationToken = default)
    {
        await _process.WindowSetSizeAsync(Identifier, width, height, cancellationToken);
    }

    public async Task<(int Width, int Height)> GetSizeAsync(CancellationToken cancellationToken = default)
    {
        return await _process.WindowGetSizeAsync(Identifier, cancellationToken);
    }

    public async Task CloseAsync(bool forceClose = false, CancellationToken cancellationToken = default)
    {
        await _process.WindowCloseAsync(Identifier, forceClose, cancellationToken);
    }

    public async Task SetFullscreenAsync(bool fullscreen, CancellationToken cancellationToken = default)
    {
        await _process.WindowSetFullscreenAsync(Identifier, fullscreen, cancellationToken);
    }

    public async Task RequestFocusAsync(CancellationToken cancellationToken = default)
    {
        await _process.RequestFocusAsync(Identifier, cancellationToken);
    }

    public async Task SetDevelopmentToolsEnabledAsync(bool developmentToolsEnabled,
        CancellationToken cancellationToken = default)
    {
        await _process.WindowSetDevelopmentToolsEnabledAsync(Identifier, developmentToolsEnabled, cancellationToken);
    }

    public async Task SetDevelopmentToolsVisibleAsync(bool developmentToolsVisible,
        CancellationToken cancellationToken = default)
    {
        await _process.WindowSetDevelopmentToolsVisibleAsync(Identifier, developmentToolsVisible, cancellationToken);
    }

    public async Task<(bool Success, byte[] Data)> ExecuteDevToolsMethodAsync(string methodName, string? json = null,
        CancellationToken cancellationToken = default)
    {
        return await _process.WindowExecuteDevToolsMethodAsync(Identifier, methodName, json, cancellationToken);
    }
    ...
    public async Task CenterSelfAsync(CancellationToken cancellationToken = default)
    {
        await _process.WindowCenterSelfAsync(Identifier, cancellationToken);
    }
    ...
    public async Task SetModifyRequestsAsync(bool modifyRequests, bool modifyBody,
        CancellationToken cancellationToken = default)
    {
    ...
    public void InvokeOnFocused()
    {
        OnFocused?.Invoke();
    }

    public void InvokeOnUnfocused()
    {
        OnUnfocused?.Invoke();
    }

    public void InvokeOnFullscreenChanged(bool fullscreen)
    {
        OnFullscreenChanged?.Invoke(fullscreen);
    }

    public void InvokeOnLoadStart(string? url)
    {
        OnLoadStart?.Invoke(url);
    }

    public void InvokeOnLoadEnd(string? url)
    {
        OnLoadEnd?.Invoke(url);
    }

    public void InvokeOnLoadError(int errorCode, string? errorText, string? failedUrl)
    {
        OnLoadError?.Invoke(errorCode, errorText, failedUrl);
    }

    public void InvokeOnDevToolsEvent(string? method, byte[] parameters)
    {
        OnDevToolsEvent?.Invoke(method, parameters);
    }

    public void WaitForExit()
    {
        _closeCompletionSource.Task.Wait();
    }

    public async Task WaitForExitAsync(CancellationToken cancellationToken = default)
    {
        await Task.WhenAny(_closeCompletionSource.Task, Task.Delay(-1, cancellationToken));
    }
    ...
    public async Task<IPCResponse?> ProxyRequestAsync(IPCRequest request)
    {
        ...
        {
            Logger.Error<DotCefWindow>("Exception occurred while processing request proxy", ex);
            Debugger.Break();
            Task.Run(async delegate
            {
                ...
                {
                    Logger.Error<DotCefWindow>(
                        "Exception occurred while trying to close window after request proxy exception.", ex2);
                }
            });
    ...
    public IPCRequest? ModifyRequest(IPCRequest request)
    {
        try
        {
            if (_requestModifier != null) return _requestModifier(this, request);
            return request;
        }
        catch (Exception ex)
        {
            Logger.Error<DotCefWindow>("Exception occurred while processing modify request", ex);
            Debugger.Break();
            Task.Run(async delegate
            {
                try
                ...
                {
                    Logger.Error<DotCefWindow>(
                        "Exception occurred while trying to close window after modify request exception.", ex2);
                }
            });
            return request;
        }
